...
def mark_attendance():
    ...
    # Check for existence of students
    if os.path.exists(CSV_FILE):
        global df
        df = pd.read_csv(CSV_FILE, dtype={'Student Name': str, 'UID': str})
    else:
    ...
    # A blank list to store the valye of attendance
    attendance_vars = []
    
    # Iterate through each row (student) in the DataFrame
    for index, row in df.iterrows():
        tk.Label(attendance_window, text=row['Student Name'], font=('comicsansms', 12),padx=10).grid(row=index + 1, column=0)
    ...
    # Function to save the current attendance
    def save_attendance():
...
def remove_student():
    ...
    # Function to remove a student from CSV file
